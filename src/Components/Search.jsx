...
export default function Search() {
    const [searchTerm, setSearchTerm] = useState('');const filteredItems = items
  .filter((menu) => menu.name.replace(/\s+/g, "").toLowerCase().includes(searchTerm.replace(/\s+/g, "").toLowerCase()))
  .slice(0, 10);
  ...
  return (
    ...
    <div className=" bg-crisp-white w-[80%]  left-10 absolute top-[2rem] ">
     <div className="flex items-center  flex-col gap-2 mt-[2rem]">
              ...
              <div key={item.id} className="flex justify-between w-full items-center bg-crisp-white p-3  border-primary border-b-[0.5px] ">
                <img
                  src={item.image}
                  alt={item.name}
