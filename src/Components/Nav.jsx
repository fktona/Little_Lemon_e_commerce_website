...
function Nav() {
  const { profileInformation, memoizedUserData, setShowLoggedIn , allOrder , aboutToSignOut , authUser} = useContext(UserContext);
...
const location = useLocation(); // Get the current useLocation
  ...
  const toggleLogin = () => {
    authUser? (setShowLoggedIn(false), setSignoutConfirm(true)):setShowLoggedIn(true)
  };
       ...
       b-crisp-white top-0  bg-clip-padding  backdrop-blur-lg
        ...
        {authUser ? (
          <div className={`text-accent block ${shouldHighlightLink('/about') ? 'text-secondary' : 'text-accent'}`}>
            HI, {profileInformation ? memoizedUserData.firstname:''}
          </div>
        ):null}
...
{ signoutConfirm ? <div className="absolute shadow-outline flex flex-col items-center gap-3 justify-center z-[12] right-3 top-10 w-[40%] p-4 bg-crisp-white"> <MdClose onClick ={()=> setSignoutConfirm(false) } className="bg-red-500 absolute top-2 right-2 text-white" /> Signout? <button onClick ={ () => {
   aboutToSignOut()
   setSignoutConfirm(false)
}
} className= "py-1 px-2 text-white bg-accent mx-auto">Yes </button></div>:''}
