...
const DeliveryDetails = () => {
  ...
  const { addresses , defaultAddress , setDefaultAddress , allOrder} = useContext(UserContext);
 ...
 const deliveryAddress = addresses[defaultAddress]
  ...
  return (
    ...
    <NavLink to="/addresses/">      
     <button className="p-2 text-sm relative float-right text-white bg-secondary shadow-md"> change Address </button>
    </NavLink>
      <ul>
...
{Object.entries(deliveryAddress.data).map(([key, value]) => (
    ...
    </div>:        <NavLink className="w-full relative flex justify-center" to="/addresses/add">      
     <button className="bg-secondary text-white  p-2 px-3 font-bold shadow-md  hover:bg-accent w-fit mx-auto relative transform hover:translate-y-6px transition transition transform duration-700 linear " > Add Address</button>
    </NavLink> }
        <h2 className="text-md relative text-slate-500 shadow-sm  text-xl  font-semibold mt-4 p-4"> Orders</h2>
    ...
    <h2 className="text-md relative text-slate-500 shadow-sm  text-xl  font-semibold mt-4 p-4">Select Payment Methood</h2>                 
<div className="relative flex justify-evenly p-2 mt-4">
   ...
   <div
    className="bg-secondary text-white text-lag py-2 px-3 font-bold shadow-lg inset-x-0 hover:bg-accent hover:shadow-lg mt-3 flex self-center w-fit mx-auto relative transform hover:translate-y-6px transition transition transform duration-700 linear " > <Payment> Proceed To Payment </Payment> 
    </div>
    </div>
  );
}
