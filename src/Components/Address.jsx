...
const AddressList = () => {
  ...
  const { addresses , defaultAddress , setDefaultAddress} = useContext(UserContext);
  ...
  return (
    <div className="flex mt-4 flex-col items-center">
       ...
       className="p-[3px] px-3 text-center text-white relative text-sm bg-accent shadow-md"><MdWest /></button>
      {addresses.length > 0 ? (
        ...
        {Object.entries(address.data).map(([key, value]) => (
            ...
            <button
            disable = { defaultAddress === index ? true :false}
            onClick={() => handleSetDefaultAddress(index)}
            className='w-fit py-2 px-3  bg-primary  text-white mx-auto'>
                       {defaultAddress ===index ?   'Default Address' :'set as default address'}
              </button>
    ...
    <NavLink to="/addresses/add">      
      <button className="border border-1 border-primary p-2 text-primary relative shadow-outline mt-5 mx-auto font-bold"> Add New Address </button>        
    </NavLink>
    </div>
  );
};
